...
export default {
  ...
  methods: {
    ...
    handleChange (currentValue, oldValue, index) {
      ...
      this.$axios.post('http://localhost:8088/changecount', formData, config)
        .then(res => {
        })
    },
    pay () {
    ...
    onSubmit () {
      if (this.money > sessionStorage.getItem('money')) {
        ...
        this.$axios.post('http://localhost:8088/pay', formData, config)
          .then(res => {
            this.getData()
    ...
    getData () { // 后台获取数据
      ...
      this.$axios.post('http://localhost:8088/car', formData, config)
        .then(res => {
    ...
    sure () { // 将商品从购物车删除
      ...
      this.$axios.post('http://localhost:8088/dgoodcar', formData, config)
        .then(res => {
          this.$notify({
            title: '成功',
            message: '操作成功',
