...
export default {
  ...
  methods: {
    ...
    getData () { // 后台获取数据
      ...
      this.$axios.post('http://localhost:8088/tui', formData, config)
        .then(res => {
    ...
    sure1 () { // 拒绝退款
      ...
      this.$axios.post('http://localhost:8088/jutui', formData, config)
        .then(res => {
          this.getData()
          this.$notify({
            title: '成功',
    ...
    sure () { // 同意退款
      ...
      this.$axios.post('http://localhost:8088/allowtui', formData, config)
        .then(res => {
          this.getData()
          this.$notify({
            title: '成功',
