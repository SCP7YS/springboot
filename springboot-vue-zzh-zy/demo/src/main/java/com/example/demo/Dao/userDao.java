...
public interface userDao {
	...
	@Insert("INSERT into user(id,word,name,sex,phone,email,city,account,status) "   //注册用户信息插入待验证用户表
			+ "values(#{0},#{1},#{2},#{3},#{4},#{5},#{6},#{7},#{8}) ")
	void insertZhuce(String id,String word,String name,String sex,String phone,String email,String city,String account,String status);

	@Select("SELECT * FROM user where id = #{0}")
    List<User> getZhuceUser(String id);      //验证用户注册是否重复提交
	
	@Select("SELECT * FROM user where status = '4'")
    List<User> getUser();      //获取待验证的注册用户列表
	
	@Delete("Delete  FROM user WHERE id = #{0} and status = '4'")
    void deleteUser(String id);      //删除允许登录的注册用户

	@Update("UPDATE user SET status = '1',money = '0',grade = '0' where id = #{0}")
